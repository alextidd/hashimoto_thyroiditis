...
```{r tree}
library(ape)
tree_file <- "/lustre/scratch125/casm/teams/team268/at31//projects/hashimoto_thyroiditis/out/resolveome/basejumper/bj-somatic-variantcalling/dna/PD63118/PD63118_run/SOMATIC_VARIANT_WORKFLOW_Heuristic_Filter_SEQUOIA/Sequoia_group_null_bino-10_rhosnp0.4_rhoindel0.4_mincov10_maxcov500_both_tree_with_branch_length.tree"
tree <-
  tree_file %>%
  ape::read.tree()
...
cell_annots <-
  ...
  dplyr::left_join(
    "/lustre/scratch125/casm/teams/team268/at31//projects/hashimoto_thyroiditis/out/resolveome/basejumper/bj-somatic-variantcalling/dna/PD63118/samplesheet.csv" %>%
      readr::read_csv() %>%
      dplyr::transmute(id = biosampleName, cell_id = gsub("_dna.*", "", id)) %>%
      ...
      dplyr::left_join(
        readr::read_tsv("data/resolveome/manual_inspection/pta_additional_annotation_H1.tsv") %>%
          dplyr::mutate(cell_id_new = cell_ID, cell_id = well_ID)
      )
  )

# prune tips not in the new cell ids
to_prune <- cell_annots %>% dplyr::filter(is.na(cell_id_new)) %>% dplyr::pull(id)
pruned_tree <- drop.tip(tree, to_prune)
cell_annots_pruned <-
  cell_annots %>%
  dplyr::filter(!is.na(cell_id_new))

# rename tips
pruned_tree$tip.label <- cell_annots_pruned$cell_id_new

# plot tree
pdf(paste0("out/resolveome/sequoia/tree_with_branch_length.pdf"), width = 10)
