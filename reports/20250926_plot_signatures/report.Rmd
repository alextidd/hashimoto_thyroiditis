...
sigs_of_interest <-
  c("machado_2022_SBSblood", "SBS9", "SBS17", "lodato_2018_ScB", "machado_2022_SignatureIg")
```
...
# load ref
ref <-
  read.table("out/resolveome/signatures/cosmic_v3.4_ScF_ScB_SBSblood.tsv") %>%
  t() %>%
  as.data.frame()
...
test <-
  ...
  purrr::map(function(i) {
    ...
    deconstructSigs::whichSignatures(
      ...
      signatures.ref = ref[sigs_of_interest, ],
      sample.id = i,
...
sigs_exposures <-
  test %>%
  purrr::map(~ .x$weights %>% mutate(unknown = .x$unknown)) %>%
  bind_rows() %>%
  t()
...
```{r burdens_per_sig, fig.height = 10}
# prep data
p_dat <-
  dat %>%
...
purrr::walk2(names(muts_per_ct), muts_per_ct, function(ct, muts_per_ct_i) {
  p <-
    plot_spectrum_per_group(muts_per_ct_i, ct)
  print(p)
})
```
...
purrr::walk2(names(muts_per_cell), muts_per_cell, function(cell_ID, muts_per_cell_i) {
  p <-
    plot_spectrum_per_group(muts_per_cell_i, cell_ID)
  print(p)
})
```
