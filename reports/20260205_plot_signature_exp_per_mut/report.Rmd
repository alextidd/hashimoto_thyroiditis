...
purrr::walk2(names(exp_per_cell), exp_per_cell, function(sig, p_dat) {
  p <-
    p_dat %>%
    dplyr::filter(metric == "n mutations") %>%
...
shm_per_cell %>%
  ...
  dplyr::left_join(cell_annots %>% dplyr::distinct(celltype_mut, well_id)) %>%
