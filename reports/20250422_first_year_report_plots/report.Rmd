...
final_cells <-
  ...
  readr::read_tsv() %>%
  dplyr::rename(cell_id_new = cell_ID, cell_id = well_ID)
...
mosdepth <-
  ...
  transmute(
    ...
    out_dir = file.path("out/resolveome/nf-resolveome", seq_type, donor_id, "cells", id),
...
cov <-
  xfun::cache_rds({
    cov <-
      ...
      transmute(id, name = "mean coverage", value = mean, seq_type)
    cov
...
perc_cov <-
  dist %>%
  filter(cov == 1) %>%
  transmute(id, name = "% genome covered", value = 100 * prop, seq_type)
```
...
snps <-
  ...
  purrr::set_names() %>%
  purrr::map(~
    read_tsv(file.path("out/resolveome/nf-resolveome", .x,
              "PD63118/genotyping/snps/PD63118_genotyped_snps.tsv"))) %>%
  bind_rows(.id = "seq_type") %>%
  group_by(seq_type)
snps
saveRDS(snps, "reports/20250422_first_year_report_plots/snps.rds")

# snps <- readRDS("reports/20250422_first_year_report_plots/20251021/20251021_first_year_report_plots_cache/snps_260c0696fa9f5a4d63ffd1a57a448e30.rds")
# snps <-
#   snps %>%
#   split(.$seq_type)
# snps$dnahyb <-
#   snps$dnahyb %>%
#   dplyr::select(-c(mut_depth, mut_vaf, donor_id, ref_depth))
# snps$dna <-
#   snps$dna %>%
#   dplyr::mutate(alt_depth = mut_depth, alt_vaf = mut_vaf) %>%
#   dplyr::select(-c(mut_depth, mut_vaf, donor_id, ref_depth))
# snps <- bind_rows(snps)
# saveRDS(snps, "reports/20250422_first_year_report_plots/snps.rds")
...
saveRDS(p_snps_dat, "reports/20250422_first_year_report_plots/p_snps_dat.rds")
...
saveRDS(allelic_statuses, "reports/20250422_first_year_report_plots/allelic_statuses.rds")
```

```{r plot_allelic_vaf_dist, fig.height = 8}
p_snps_dat <-
  readRDS("reports/20250422_first_year_report_plots/p_snps_dat.rds")
p_snps_dat %>%
  ggplot(aes(x = alt_vaf_mid, y = n, fill = total_depth_bin)) +
  geom_col() +
...
allelic_statuses <-
  readRDS("reports/20250422_first_year_report_plots/allelic_statuses.rds")
allelic_dropout <-
  allelic_statuses %>%
...
allelic_statuses %>%
  ...
  left_join(
    allelic_statuses %>%
      ...
      ungroup() %>%
      transmute(cell_id = gsub("_dna.*", "", id), arr = prop)) %>%
...
We filter to `alt_vaf` > 0.2 and `alt_depth` > 1.
...
p_dat %>%
  ...
  right_join(
    p_dat$`% of activated B cells with a driver` %>% dplyr::select(gene, prop = n)) %>%
...
genos %>%
  left_join(ss) %>%
  filter(alt_vaf > 0) %>%
  ggplot(aes(x = alt_vaf, fill = on_1p & loh_1p == "1")) +
  geom_histogram() +
  theme_classic() +
  scale_y_continuous(expand = c(0, 0)) +
...
genos_filter %>%
  ...
  group_by(groups) %>%
  summarise(mean = mean(alt_vaf, na.rm = T), min = min(alt_vaf, na.rm = T),
            max = max(alt_vaf, na.rm = T))
```
  ...
  bj_muts_per_seq_type_38 <-
    ...
    purrr::map(function(seq_type) {
      ...
      c("alt_depth" = "NV", "total_depth" = "NR") %>%
        ...
        group_by(mut_id) %>%
        mutate(global_alt_depth = sum(alt_depth),
               global_total_depth = sum(total_depth),
               global_alt_vaf = global_alt_depth / global_total_depth) %>%
        ungroup() %>%
        # get muts only
        filter(alt_depth > 0)
    }) %>%
...
```{r plot_global_vaf}
bj_muts_per_seq_type %>%
  distinct(chr, mut_id, seq_type, global_alt_vaf) %>%
  ggplot(aes(x = global_alt_vaf)) +
...
bj_muts_per_seq_type_filter %>%
  distinct(chr, alt_id, seq_type, global_alt_vaf) %>%
  ggplot(aes(x = global_alt_vaf)) +
  geom_histogram(bins = 100) +
  lims(x = c(NA, 1)) +
  ggh4x::facet_nested(seq_type ~ ., scales = "free_y") +
...
bj_muts <- xfun::cache_rds({
  bj_muts_per_seq_type_filter %>%
    ...
    group_by(across(-c(id, ends_with("_depth"), global_alt_vaf))) %>%
...
We filter to `alt_vaf` > 0.3 and `alt_depth` > 5, and present in less than 50% 
of cells.
...
bj_muts_filter <-
  xfun::cache_rds({
    bj_muts %>%
      filter(alt_vaf > 0.3, alt_depth > 5) %>%
      group_by(chr, pos, ref, alt) %>%
...
tree <-
  ...
  purrr::map(function(seq_type) {
    # read tree
    tree <-
      file.path("out/BaseJumper/bj-somatic-variantcalling/", seq_type,
              "/PD63118_run/SOMATIC_VARIANT_WORKFLOW_Heuristic_Filter_SEQUOIA/") %>%
...
expr <-
  "out/BaseJumper/bj-expression/rna/PD63118_run/secondary_analyses/quantification_htseq/matrix_gene_counts_starhtseq.txt" %>%
  readr::read_tsv() %>%
...
expr_counts <-
  expr %>%
  dplyr::select(gene_id, gene_symbol, any_of(cells$sample_id)) %>%
