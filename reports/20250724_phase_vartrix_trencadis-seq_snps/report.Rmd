...
purrr::walk2(names(mats_filter), mats_filter, function(sample, x) {
  purrr::walk2(names(x), x, function(type, m) {
    ...
    if (length(missing) > 0) {
      zero_mat <- Matrix(0, nrow = nrow(m), ncol = length(missing),
                         sparse = TRUE)
...
mat_long %>%
  dplyr::mutate(
    alt_vaf_bin = cut(alt_vaf, breaks = seq(0, 1, by = bin_int),
                      include.lowest = TRUE, labels = FALSE),
    alt_vaf_mid = ((alt_vaf_bin - 1) * bin_int + alt_vaf_bin * bin_int) / 2) %>%
...
p_dat <-
  ...
  summarise(total_depth_1p = sum(total_depth), .groups = "drop")

# total depth per barcode
p_dat %>%
  ggplot(aes(x = total_depth_1p)) +
  geom_histogram(binwidth = 1) +
  ggtitle("total depth per barcode")

# total depth per barcode per celltype
p_dat %>%
  ggplot(aes(x = total_depth_1p)) +
  geom_histogram() +
  geom_text(
    data = p_dat %>%
      mutate(x = max(total_depth_1p * 0.9), y = Inf) %>%
      group_by(celltype, x, y) %>%
      summarise(n_cells_10_reads = sum(total_depth_1p >= 10)),
    aes(x = x, y = y, label = paste0(n_cells_10_reads, " cells w 10+ reads")),
    vjust = 2, hjust = 1) +
  geom_vline(xintercept = 10) +
  facet_wrap(~ celltype) +
  ggtitle("total depth per barcode per celltype")
```
