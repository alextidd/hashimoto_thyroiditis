...
```{r load_ss}
man_insp <-
  read_tsv("data/manual_inspection/PD63118.tsv") %>%
  dplyr::select(-run_id) %>%
  # fix loh_1p
...
bje %>% dplyr::count(Progenitor)

bje %>% filter(celltype_VDJ_SHM_CSR == "other") %>% dplyr::count(Progenitor)

bje %>%
  mutate(celltype_bje = case_when(
    Progenitor %in% c("B_cell", "T_cells", "NK_cell", "Macrophage", "Monocyte") ~ Progenitor,
    TRUE ~ "other")) %>%
  dplyr::count(celltype_VDJ_SHM_CSR, celltype_bje)
```

```{r ct_stats}
ct_stats <-
  cts %>% dplyr::count(celltype_VDJ_SHM_CSR) %>% {split(.$n, .$celltype_VDJ_SHM_CSR)}
...
bje_stats <-
  ...
  dplyr::count(lineage, celltype, celltype_bje)
...
saveRDS(p_snps_dat, "reports/20250422_first_year_report_plots_cache/p_snps_dat.rds")
...
saveRDS(allelic_statuses, "reports/20250422_first_year_report_plots_cache/allelic_statuses.rds")
```

```{r plot_allelic_vaf_dist}
# plot
p_snps_dat <-
  readRDS("reports/20250422_first_year_report_plots_cache/p_snps_dat.rds")
p_snps_dat %>%
...
```{r plot_allelic_dropout}
# get prop allelic dropout
allelic_statuses <-
  readRDS("reports/20250422_first_year_report_plots_cache/allelic_statuses.rds")
allelic_dropout <-
  allelic_statuses %>%
  filter(allelic_status %in% c("one", "neither")) %>%
  group_by(id) %>%
  summarise(id, name = "allelic dropout", value = sum(prop))
...
p_cov_dat <-
  bind_rows(read_counts, gene_counts, cov, perc_cov, allelic_dropout) %>%
  inner_join(ss_annot) %>%
...
`r round(seq_stats$dna[["% genome covered"]]$mean)`%
for DNA genome-wide, and
`r round(seq_stats$dnahyb[["% panel covered"]]$mean)`%
...
bj_muts_per_seq_type <- xfun::cache_rds({
  ...
  bj_muts_per_seq_type <-
    ...
    mutate(pos = pos_37, mut_id = paste(chr, pos, ref, mut, sep = "_")) %>%
    select(-pos_37)
...
We plot a heatmap of all mutations.

```{r}
plot_vaf_heatmap(bj_muts_per_seq_type %>% mutate(alt = mut))
```
...
muts_genos_overlap <-
  full_join(
    bj_muts_filter %>%
      transmute(chr = gsub("^chr", "", chr), pos, ref, mut, cell_id,
                bj_muts = "bj_muts") %>%
      distinct(),
    genos_filter %>%
      transmute(chr, pos, ref, mut = alt, cell_id, gene, genos = "genos") %>%
      distinct()) %>%
