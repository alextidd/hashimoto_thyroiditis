...
trinucs <-
  c("true", "artefact") %>%
  purrr::set_names() %>%
  purrr::map(function(class_i) {
    classed_muts %>%
      dplyr::filter(class == class_i, nchar(ref) == 1, nchar(alt) == 1) %>%
      split(.$chr) %>%
      purrr::map(function(dat_i) {
        dat_i %>%
          ...
          tidyr::pivot_longer(cols = -id, names_to = "sub",
                              values_to = "count") %>%
          dplyr::mutate(trinuc_sub = substr(sub, 3, 5),
                        trinuc = paste0(substr(sub, 1, 1), substr(sub, 3, 3),
                                        substr(sub, 7, 7)))
      }) %>%
...
# plot trinucs by chromosome
trinucs %>%
  dplyr::filter(class == "artefact") %>%
  split(.$chr) %>%
  purrr::map(~ plot_trinucs(.x) +
               ggtitle(paste("Artefactual variants (VAF < 0.75):", unique(.x$chr))))
```
...
# plot trinucs
plot_trinucs(artefact_trinucs_lo) +
