...
```{bash construct_vcfs, eval = FALSE}
# modules
...
## Step 2: Run matrix generator

```{bash run_matrix_generator, eval = FALSE}
module load python-3.11.6/perl-5.38.0 openjdk-11.0.20.1_1
module load sigprofiler/extractor-1.1.24-GRCh38-GRCh37
module load sigprofiler/matrixGenerator-1.2.30
...
```{bash construct_mutational_spectra_plots, eval = FALSE}
...
## Step 3: Extract mutational signatures

### Option A: SigProfiler Extractor

```{bash sigprofiler_extract, eval = FALSE}
module load python-3.11.6/perl-5.38.0 openjdk-11.0.20.1_1
module load sigprofiler/extractor-1.1.24-GRCh38-GRCh37
module load sigprofiler/matrixGenerator-1.2.30
...
#### SigProfiler extractor script

```{bash sigprofiler_extractor_script, eval = FALSE}
#!/usr/bin/env python
import sys, argparse
...
```{bash load_msighdp_libraries, eval = FALSE}
# load r and r_libs :
module load r-4.2.2 
...
#### Reformat input matrix

```{r reformat_input_matrix, eval = FALSE}
# reformat the indels matrix
library(stringr)
library(dplyr)
...
## Step 5: Attribute mutational signatures

```{bash attribute_signatures, eval = FALSE}
# load modules
module load python-3.11.6/perl-5.38.0 openjdk-11.0.20.1_1
module load sigprofiler/extractor-1.1.24-GRCh38-GRCh37
module load sigprofiler/matrixGenerator-1.2.30
