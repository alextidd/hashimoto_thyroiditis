...
# rmarkdown::render('src/03c_analyse_genotyping.Rmd', output_file = 'analyse_genotyping.html', output_dir = 'out/genotyping/49882')
...
muts_and_snps <-
  ...
  purrr::map(function(x) {
    list.files(paste0("out/genotyping/49882/", x, "_genotyped_by_chr/Q30/"),
               pattern = ".tsv$", full.names = TRUE) %>%
      purrr::map(function(y) {
        y %>%
          readr::read_tsv(col_types = list(chr = readr::col_character(),
                                           ref = readr::col_character(),
                                           mut = readr::col_character()))
      }) %>%
...
seq(1, 5) %>%
  purrr::map(function(min_depth) {
    p_dat <-
      muts_and_snps %>%
      dplyr::filter(total_depth >= min_depth, source == "nanoseq_mutations",
                    mut_vaf > 0)
    p_dat %>%
      ggplot(aes(x = mut_vaf)) +
      geom_histogram(bins = 100) +
      ggtitle(paste0("Distribution of non-zero VAFs in PTA samples, min ",
                    min_depth, " total reads (",
