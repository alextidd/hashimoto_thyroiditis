...
echo 'R_LIBS=/nfs/casm/team268im/at31/R/R-tmp-4.4:/software/conda/users/at31/scan2/lib/R/library:/software/isg/languages/R/4.4.0/exec/lib/R/library' > ~/.Renvironexport R_LIBS=/nfs/casm/team268im/at31/R/R-tmp-4.4:/software/conda/users/at31/scan2/lib/R/library
export R_LIBS_USER=/nfs/casm/team268im/at31/R/R-tmp-4.4:/software/conda/users/at31/scan2/lib/R/library
...
(
  ...
  $scan2_dir/bin/scan2 \
    ...
    call_mutations \
    --joblimit 1000 \
    --cluster "bsub -J {name} -q basement -n {threads} -M {resources.mem_mb} -R 'select[mem>{resources.mem_mb}] rusage[mem={resources.mem_mb}] span[hosts=1]' -W 336:00 -o %logdir/%J_{name}.out -e %logdir/%J_{name}.err -env 'all'" \
