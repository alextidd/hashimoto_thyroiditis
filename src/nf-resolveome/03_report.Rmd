...
# dirs
out_dir <- paste0("out/nf-resolveome/reports/", donor_id_i, "/")
...
man_insp <-
  read_tsv(paste0("data/resolveome/manual_inspection/", donor_id_i, ".tsv")) %>%
  dplyr::select(-run_id) %>%
  # fix loh_1p
...
ss <-
  file.path(Sys.getenv("LUSTRE_TEAM"),
            "resolveome/data/bams/samplesheet_local.csv") %>%
  read_csv() %>%
  # add manual inspection data
  left_join(man_insp) %>%
  filter(donor_id == donor_id_i)
```
...
mosdepth <-
  ...
  transmute(
    ...
    out_dir = file.path("out/nf-resolveome", seq_type, donor_id, "cells", id),
...
snps <-
  xfun::cache_rds({
    snps <-
      ...
      purrr::set_names() %>%
      purrr::map(~
        read_tsv(paste0("out/nf-resolveome/", .x, "/", donor_id_i,
                        "/genotyping/snps/", donor_id_i,
                        "_genotyped_snps.tsv"))) %>%
...
saveRDS(p_snps_dat, paste0(out_dir, "/report_cache/p_snps_dat.rds"))
...
saveRDS(allelic_statuses, paste0(out_dir, "/report_cache/allelic_statuses.rds"))
```

```{r plot_allelic_vaf_dist, fig.height = 8}
p_snps_dat <-
  readRDS(paste0(out_dir, "/report_cache/p_snps_dat.rds"))
p_snps_dat %>%
...
allelic_statuses <-
  readRDS(paste0(out_dir, "/report_cache/allelic_statuses.rds"))
allelic_dropout <-
  allelic_statuses %>%
...
ss_annot %>%
  ...
  readr::write_tsv(paste0("out/analysis/", donor_id_i,
                          "_TNFRSF14_mut_status.tsv"))
genos_filter %>%
  readr::write_tsv("out/analysis/", donor_id_i,
                   "_filtered_genotyped_mutations.tsv")
      ...
      c("mut_depth" = "NV", "total_depth" = "NR") %>%
        purrr::map(function(i) {
          paste0("out/BaseJumper/bj-somatic-variantcalling/", seq_type, "/",
                 donor_id_i,
                 "_run/SOMATIC_VARIANT_WORKFLOW_Heuristic_Filter_SEQUOIA", 
                "/Sequoia_group_", donor_id_i,
                "_bino-10_rhosnp0.4_rhoindel0.4_mincov10_maxcov500_both_",
                i, "_filtered_all.txt") %>%
            read.table() %>%
  ...
  purrr::map(function(seq_type) {
    ...
    tree <-
      paste0("out/BaseJumper/bj-somatic-variantcalling/", seq_type, "/",
             donor_id_i,
             "_run/SOMATIC_VARIANT_WORKFLOW_Heuristic_Filter_SEQUOIA/") %>%
      list.files(pattern = "_both_tree_with_branch_length.tree",
                 full.names = TRUE) %>%
      ape::read.tree()
